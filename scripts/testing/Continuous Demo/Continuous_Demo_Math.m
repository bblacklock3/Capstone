function[x, Angle, End_Slope] = Continuous_Demo_Math(P, Joint, X_Start, Length, Angle_Start, Angle_Diff, Angle_End, smooth, isflat, Init_Slope, Angle_Des)
...
if isflat == true
    ...
    Angle(length(x_smooth)+1:end) = Angle(length(x_smooth)+1:end) + Offset;
    Angle_End = Angle(end) + Angle_Start;
    Angle_End_Flip = -Angle(end) + Angle_Start;
    if abs(Angle_End - Angle_Des) > abs(Angle_End_Flip - Angle_Des)
        Angle = -Angle;
    end
