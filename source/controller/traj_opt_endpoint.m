...
tic
height_error_min = @(x) (kneePeakPos-x(gridN))^2;
% The initial parameter guess; gridN positions, gridN velocities,
% gridN accelerations
x0 = [1; linspace(kneeCurrPos,kneePeakPos,gridN)'; linspace(kneeCurrVel,kneeCurrVel,gridN)'; zeros(gridN, 1)];
...
ub = [max_time;  ones(gridN * 2, 1) * Inf;   ones(gridN, 1) * 30];
% Options for fmincon
...
x_optimal = fmincon(height_error_min, x0, A, b, Aeq, Beq, lb, ub, ...
...
sim_time = x_optimal(1);
delta_time = sim_time / gridN;
times = 0 : delta_time : sim_time - delta_time;

[positions, vels, accs] = get_traj(x_optimal);
...
function [ c, ceq ] = double_integrator_constraints( x )
    ...
    ceq = [ceq ; positions(end) - kneePeakPos; vels(end)-10];
% figure(1);
% plot(accs);
% title('Control Input (Acceleration) vs Time');
% xlabel('Time (s)');
